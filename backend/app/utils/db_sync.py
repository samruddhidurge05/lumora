...
def get_product_by_id(db_session, product_id: int) -> Product:
    ...
    try:
        ...
        new_prod = Product(
            ...
            owner_type=data.get("owner_type") or ("PLATFORM" if data.get("vendor_id") in ("lumora-creator", "", None) or product_id in (108,109,110,111,112,115,116,117,118,119,120,121,122) else "VENDOR"),
            created_by_role=data.get("created_by_role") or ("ADMIN" if (data.get("vendor_id") in ("lumora-creator", "", None) or product_id in (108,109,110,111,112,115,116,117,118,119,120,121,122)) else "VENDOR"),
            is_platform_product=bool(data.get("is_platform_product") if data.get("is_platform_product") is not None else (data.get("vendor_id") in ("lumora-creator", "", None) or product_id in (108,109,110,111,112,115,116,117,118,119,120,121,122))),
