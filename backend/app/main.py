...
def _run_schema_migrations() -> None:
    ...
    if dialect == "postgresql":
        ...
        pg_migrations = [
            ...
            "CREATE TABLE IF NOT EXISTS platform_withdrawals (id SERIAL PRIMARY KEY, withdrawal_number VARCHAR(64) UNIQUE NOT NULL, amount FLOAT NOT NULL, currency VARCHAR(10) NOT NULL DEFAULT 'INR', status VARCHAR(30) NOT NULL DEFAULT 'pending', requested_by INTEGER NOT NULL REFERENCES users(id), requested_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, approved_by INTEGER REFERENCES users(id), approved_at TIMESTAMP, completed_by INTEGER REFERENCES users(id), completed_at TIMESTAMP, transaction_reference VARCHAR(120), destination_type VARCHAR(50) NOT NULL DEFAULT 'bank_account', destination_account TEXT, notes TEXT, failure_reason TEXT, created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)",
            # platform_withdrawals — Phase 2/3 additive columns
            "ALTER TABLE platform_withdrawals ADD COLUMN IF NOT EXISTS completed_by INTEGER REFERENCES users(id)",
            "ALTER TABLE platform_withdrawals ADD COLUMN IF NOT EXISTS razorpayx_payout_id VARCHAR(120)",
            "ALTER TABLE platform_withdrawals ADD COLUMN IF NOT EXISTS razorpayx_fund_account_id VARCHAR(120)",
    ...
    if dialect == "sqlite":
        try:
            with engine.connect() as conn:
                ...
                # platform_withdrawals — Phase 2/3 additive columns (idempotent)
                try:
                    pw_cols = {row[1] for row in conn.execute(_text("PRAGMA table_info(platform_withdrawals)"))}
                    pw_additions = [
                        ("completed_by",              "INTEGER REFERENCES users(id)"),
                        ("razorpayx_payout_id",        "VARCHAR(120)"),
                        ("razorpayx_fund_account_id",  "VARCHAR(120)"),
                    ]
                    for col_name, col_def in pw_additions:
                        if col_name not in pw_cols:
                            conn.execute(_text(f"ALTER TABLE platform_withdrawals ADD COLUMN {col_name} {col_def}"))
                            _logger.debug("[startup] SQLite platform_withdrawals.%s added OK", col_name)
                except Exception as _pw_err:
                    _logger.debug("[startup] SQLite platform_withdrawals additive columns skipped: %s", _pw_err)
