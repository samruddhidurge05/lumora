...
function PayoutReviewDrawer({ payout, onClose, onApprove, onReject, onHold, onRetry, onSimulateSandbox, loading }) {
  ...
  const [supportingOrders, setSupportingOrders] = useState(null);
  const [ordersLoading, setOrdersLoading]       = useState(false);
  const [auditTimeline, setAuditTimeline]       = useState(null);
  const [timelineLoading, setTimelineLoading]   = useState(false);

  // Fetch real supporting orders from database
  useEffect(() => {
    if (!payout?.id) return;
  ...
  // netPayable = payout.amount from database. No UI-only adjustments.
  ...
  // Real UTR: payout.utr from bank settlement, or provider ref. Never fabricated.
          ...
          <div className="flex-1 p-6 space-y-6 overflow-y-auto">
            {activeTab === 'audit' && (
              <>
                ...
                {/* SECTION 5: PAYMENT SUMMARY — amount from database only */}
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-xl bg-[#F8F3FB] border border-[#F3EAF8] space-y-3 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    {/* Real UTR from payout.utr (set after bank settlement). Never fabricated. */}
      ...
      <div className="p-3 sm:p-6 md:p-8 space-y-6 sm:space-y-8 max-w-full overflow-x-hidden min-w-0">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6 sm:space-y-8">
            ...
            {/* Executive KPIs Grid — all values from /admin/affiliates/kpis backend.
                No fallback monetary values. Displays '—' when API unavailable. */}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              {/* Webhook Sync Rate removed — no webhook delivery log table exists.
                  "99.98%" was a hardcoded fabricated metric. */}
            </div>
          </div>
        )}
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl border border-[#F3EAF8] p-6 space-y-4 shadow-xs">
                ...
                {/* Tier counts computed from affiliate_commissions.sale_amount by /kpis endpoint.
